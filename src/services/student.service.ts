...
export const updateStudentAdmission = async (
  ...
  // Double check uniqueness of Aadhaar Hash if provided (excluding current application)
  if (aadhaarHash) {
    const duplicateAadhaar = await prisma.student.findFirst({
      where: { aadhaarHash, id: { not: applicationId } }
    });
    if (duplicateAadhaar) {
      throw new Error("An application with this Aadhaar Number already exists.");
    }
  ...
  // Double check uniqueness of CET Number if provided
  if (cetNumber && cetNumber.trim() !== "") {
    const duplicateCET = await prisma.student.findFirst({
      where: { cetNumber: cetNumber.toUpperCase(), id: { not: applicationId } }
    });
    if (duplicateCET) {
      throw new Error("An application with this CET Number already exists.");
    }
  ...
  // Double check uniqueness of DCET Number if provided
  if (dcetNumber && dcetNumber.trim() !== "") {
    const duplicateDCET = await prisma.student.findFirst({
      where: { dcetNumber: dcetNumber.toUpperCase(), id: { not: applicationId } }
    });
    if (duplicateDCET) {
      throw new Error("An application with this DCET Number already exists.");
    }
...
export const submitApplication = async (userId: number) => {
  ...
  const updated = await prisma.student.update({
    ...
    data: {
      ...
      ...(nextStatus === "RESUBMITTED" ? { resubmittedAt: new Date(), rejectionRemark: null } : {})
    }
  });
